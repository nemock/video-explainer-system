...
| D1a | **YouTube = account `34001` (Dave Saunders) + a new dedicated deep-dive series playlist.** Not 38684. **LinkedIn dropped.** |
...
- G8 — **Publish to YouTube** (account 34001 + new series playlist) via Blotato; **promote via short native snippets** on X/TikTok/IG/Threads pointing at the real returned URL.
...
### 6.9 Publishing
- FR-33 — Publish to **YouTube account `34001` (Dave Saunders)** + a **new dedicated deep-dive series playlist** (not CIRCUMVENT 38684), with chosen title, description (keyword-first hook + chapters + sourced links + live book/The Build offer figures + seeded pinned comment), packaged thumbnail.
...
- FR-40 — Post snippets **natively** via Blotato to **X (`16563`), TikTok (`42662`), Instagram (`41992`), Threads (`6021`)**, each pointing at the real YouTube URL — on X the link via **self-reply** (confirmed working) + optional takeaway thread; pin the anchor.
...
**Resolved this round:** YouTube = 34001 + new series playlist · LinkedIn dropped · FWF theme = adapt the kit + McKinsey info-design · X self-reply confirmed, promo = native snippets on X/TikTok/IG/Threads · logo/book-cover asset paths known.
...
- **Phase 3:** YouTube publishing (account 34001, new series playlist, hardened upload, packaging, end screens/cards) **+ the snippet promo engine** (3–6 <2-min native clips → X/TikTok/IG/Threads via Blotato, pointing at the captured YouTube URL).
